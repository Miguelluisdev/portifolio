...
const Header = React.memo(() => {
  ...
  return (
    <header className='flex justify-around items-center w-full h-20 text-branco fixed bg-transparent px-4 '>
      <div>
        <h1 className='text-2xl font-Montserrat font-bold ml-2 hover:text-az duration-500 shadow-AzulEscuro drop-shadow-lg shadow-2xl  '>Miguel Luis</h1>
      </div>
      ...
      <ul className={`flex flex-col justify-center items-center absolute top-0 pt-50 m-11  w-400  h-50 bg-ad border border-r-branco rounded-md  ${isMenuOpen ? 'visible' : 'invisible'}`}>
        <li className='px-4 cursor-pointer capitalize text-lg hover:text-az duration-500 text-ab shadow-AzulEscuro shadow-md '>
          <Link to="/">Home</Link>
        </li>
        <li className='px-4 cursor-pointer capitalize text-lg hover:text-az duration-500 shadow-AzulEscuro shadow-md '>
          <Link to="/Skills">Habilidades</Link>
